 ...
 */
 ...
 /*
  * Changes for Raspberry Pico Arduino IDE
  * - type tm is renamed to iec_tm to avoid conflict with existing tm type
  * - all functions using tm have been modified to use iec_tm
  */
...
// Original code with nanoseconds resolution
/*
...
*/
// New code (BEB) : use of milliseconds resolution
// if msec contains more than one millisecond, only the first millisecond is normalized
// TODO : check if it is possible that tv_msec can be more than 1999
static inline void __normalize_timespec (IEC_TIMESPEC *ts) 
{
  if( ts->tv_msec < -1000 || (( ts->tv_sec > 0 ) && ( ts->tv_msec < 0 )))
  {
    ts->tv_sec--;
    ts->tv_msec += 1000;
  }
...
static inline iec_tm convert_seconds_to_date_and_time(long int seconds) {
  iec_tm dt;
  long int days, rem;
...
#define __time_cmp(t1, t2) (t2.tv_sec == t1.tv_sec ? t1.tv_msec - t2.tv_msec : t1.tv_sec - t2.tv_sec)

static inline TIME __time_add(TIME IN1, TIME IN2){
  TIME res ={IN1.tv_sec + IN2.tv_sec,
             IN1.tv_msec + IN2.tv_msec };
...
static inline TIME __time_mul(TIME IN1, LREAL IN2){
  ...
  time_t s = (time_t)s_f;
  div_t ms = div((int)((LREAL)IN1.tv_msec * IN2), 1000);
  TIME res = {(long)s + ms.quot,
		      (long)ms.rem + (s_f - s) * 1000 };
  __normalize_timespec(&res);
  return res;
}
static inline TIME __time_div(TIME IN1, LREAL IN2){
  ...
  time_t s = (time_t)s_f;
  TIME res = {(long)s,
              (long)(IN1.tv_msec / IN2 + (s_f - s) * 1000) };
  __normalize_timespec(&res);
  return res;
}
...
static inline TIME   __real_to_time(LREAL IN) {return (TIME){IN, (IN - (LINT)IN) * 1000};}
static inline TIME __string_to_time(STRING IN){
    ...
    if(l != 0){
        ...
        return  (TIME){(long)IN_val, (long)(IN_val - (LINT)IN_val)*1000};
    }else{
...
static inline LREAL __time_to_real(TIME IN){
    return (LREAL)IN.tv_sec + ((LREAL)IN.tv_msec/1000);
}
    ...
    if(!days.rem && IN.tv_msec == 0)
	{
		...
		else
		{
			...
			else
			{
				...
				{
                    res.len = snprintf((char*)&res.body, STR_MAX_LEN, "T#%dd%dh%dm%ds%gms", days.quot, hours.quot, minuts.quot, minuts.rem, (LREAL)IN.tv_msec);
                }
            }
        }
    }
...
static inline STRING __tod_to_string(TOD IN){
    ...
    if(IN.tv_msec == 0)
	{
        ...
        res.len = snprintf((char*)&res.body, STR_MAX_LEN, "TOD#%2.2d:%2.2d:%09.6f",
                 broken_down_time.tm_hour,
                 broken_down_time.tm_min,
                 (LREAL)broken_down_time.tm_sec + (LREAL)IN.tv_msec / 1e3);
    }
    if(res.len > STR_MAX_LEN) res.len = STR_MAX_LEN;
    return res;
}
