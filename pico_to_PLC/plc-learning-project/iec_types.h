/*
Original file from MatIEC by Mario de Sousa
Modified by Benoit BOUCHEZ (BEB)
IEC_TIMESPEC is modified to use milliseconds rather than nanoseconds.
Nanoseconds resolution is useless in PLC and using milliseconds makes
the code easier to read

We also change the way the time structure is handled. For a strange reason,
original code from Mario de Sousa uses the two members of the structure to
code the same thing (tv_nsec represents the same value as tv_sec, but multiplied
by 1E9. This does not follow libc way where seconds are integer and nanoseconds
are fraction of seconds.
TODO : check how the time comparison macros are handling this approach
*/
